...
st.markdown(f"""
    ...
    /* --- CORRECTION VISIBILITÉ TEXTE SIDEBAR --- */
    /* Force TOUS les titres (h1, h2, h3) dans la sidebar à être GRIS FONCÉ */
    ...
    /* Force TOUS les textes standards, labels et spans à être GRIS FONCÉ */
    ...
    /* Exception pour les captions (petits textes) un peu plus clairs */
    ...
    .context-text {{ color: {COLOR_GRIS}; font-size: 1.05rem; font-weight: 500; line-height: 1.5; }}
...
# --- CONTEXTE AVANCÉ ET DÉTAILLÉ ---
with col_context:
    st.write("") 
    st.write("") 
    
    # Logique message contextuel DÉTAILLÉ
    if cashflow_net_mensuel > 0:
        titre_ctx = "Rentabilité Excellente"
        color_ctx = COLOR_VERT
        msg_ctx = f"""
        <b>Votre bien est un actif "payé par le locataire".</b><br><br>
        1. <b>Auto-financement total :</b> Les loyers couvrent le crédit, les charges et la gestion.<br>
        2. <b>Bonus :</b> Vous dégagez un surplus de <b style='color:{COLOR_VERT}'>{fmt(cashflow_net_mensuel*12)} €/an</b>.<br>
        3. <b>Enrichissement :</b> Chaque mois, vous remboursez du capital ET gagnez du cash.
        """
    elif cashflow_net_mensuel > -100:
        titre_ctx = "Opération Équilibrée"
        color_ctx = "#f39c12" # Orange
        msg_ctx = f"""
        <b>C'est la force du "Levier Bancaire".</b><br><br>
        Votre locataire rembourse la quasi-totalité de votre crédit à votre place.<br><br>
        Votre effort réel n'est que de <b style='color:#f39c12'>{fmt(abs(cashflow_net_mensuel))} €/mois</b> pour acquérir un patrimoine immobilier de {fmt(montant_emprunte_initial)} € (si emprunt total).<br>
        C'est un excellent ratio coût/enrichissement.
        """
    else:
        titre_ctx = "Effort d'Épargne"
        color_ctx = COLOR_ROUGE
        msg_ctx = f"""
        <b>Une stratégie de "Capitalisation Forcée".</b><br><br>
        Ce bien nécessite un effort d'épargne mensuel de <b>{fmt(abs(cashflow_net_mensuel))} €</b>.<br><br>
        Considérez cela comme un virement mensuel sur un plan d'épargne (votre crédit) qui grandit chaque mois.
        Regardez la courbe "Patrimoine" pour voir la richesse créée malgré cet effort.
        """
...
frais_details = [f"Honoraires: {fmt_dec(taux_gestion)}%"]
if gli_active: frais_details.append(f"GLI: {fmt_dec(taux_gli)}%")
...
gain_str = "un enrichissement net estimé à" if valeur_finale > 0 else "un coût global de"
...
<div class="conclusion-box no-print-shadow">
    ...
    <p class="conclusion-p">
        Pour un bien générant <b>{fmt(loyer_cc)} € CC</b> de revenus locatifs (dont {fmt(prov_mensuelle)}€ de provisions), 
        l'agence <b>PATRIM</b> sécurise votre investissement avec une offre de gestion complète. 
        Le coût mensuel global de nos services s'élève à 
        <span class="highlight-red">{fmt(total_frais_gestion)} €</span> ({frais_str}).
    </p>
    <p class="conclusion-p">
        En intégrant le remboursement de votre crédit ({fmt(credit_mensuel_ass)}€) et vos charges propriétaires, 
        votre opération dégage un résultat net mensuel de 
        <span style="color:{COLOR_VERT if cashflow_net_mensuel>=0 else COLOR_ROUGE}; font-weight:800; font-size:1.2rem;">{fmt(cashflow_net_mensuel)} €</span>.
    </p>
    <p class="conclusion-p" style="margin-bottom:0; border-top: 1px solid #eee; padding-top: 20px;">
        <b>Vision Patrimoniale à {duree_restante_annees} ans :</b><br>
        Grâce à la capitalisation de votre emprunt et aux flux de trésorerie cumulés, cette opération représente 
        {gain_str} <span style="color:{col_gain}; font-size: 1.4rem; font-weight:800;">{fmt(valeur_finale)} €</span>.
    </p>
</div>
"""
