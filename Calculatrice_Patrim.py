...
if mode_calcul == " Calculer une Mensualité":
    c1, c2 = st.columns(2)
    with c1:
        montant_projet_input = st.number_input("Montant à financer (€)", value=250000, step=1000, format="%d", help="Montant du bien + frais - apport")
        duree_annees = st.slider("Durée (années)", 7, 30, 20, 1)
    with c2:
        montant_emprunte_final = montant_projet_input
        revenus_totaux = 0
else:
    st.caption("Calcul basé sur un endettement max de 35% (assurance incluse)")
    ...
    mensualite_cible_capacite = revenus_totaux * 0.35
...
if mode_calcul == " Calculer une Mensualité":
    ...
    <div class="main-result-card">
        ...
        <div class="result-sub-val">
            Pour un emprunt de {fmt(montant_emprunte_final)} € sur {duree_annees} ans
        </div>
    </div>
    """, unsafe_allow_html=True)
    
    phrase_synthese = f"💡 **Synthèse du coût :** En empruntant **{fmt(montant_emprunte_final)} €**, le coût total du crédit (intérêts + assurance) s'élèvera à **{fmt(cout_total_credit)} €**."
    ...
    <div class="main-result-card">
        ...
        <div class="result-sub-val">
            Mensualité max : {fmt(mensualite_cc_finale)} €/mois (35% endettement)
        </div>
    </div>
    """, unsafe_allow_html=True)
    
    phrase_synthese = f"💡 **Synthèse capacité :** Avec vos revenus, vous pouvez financer un bien à hauteur de **{fmt(montant_emprunte_final)} €** (hors apport personnel et frais de notaire) sur **{duree_annees} ans**."
...
with st.container(border=True):
    ...
    if mode_calcul == " Ma Capacité d'emprunt":
        intro_mail = f"Capacité calculée sur la base de revenus totaux de {fmt(revenus_totaux)} €/mois.\n"
...
--- SYNTHÈSE ---
{intro_mail}
- Montant financé : {fmt(montant_emprunte_final)} €
