...
if mode_calcul == " Calculer une Mensualité":
    ...
    with c1:
        ...
        montant_projet_input = st.number_input("Montant du Projet (€)", value=0, step=1000, format="%d", help="Prix du bien + Frais de notaire")
        duree_annees = st.slider("Durée (années)", 1, 25, 20, 1)
    with c2:
        # AJOUT APPORT VARIABLE
        apport = st.number_input("Votre Apport (€)", value=0, step=1000, format="%d")
    ...
    with c_rev1:
        revenu_1 = st.number_input("Revenus nets mensuels (Emprunteur 1)", value=0, step=100, format="%d")
    with c_rev2:
